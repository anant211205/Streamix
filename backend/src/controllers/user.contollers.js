...
export async function getRecommendedUsers(req , res){
    try {
        ...
        console.error("error in get recommended User controllers" , error.message) ;
        res.status(500).json({
...
export async function getMyFriends(req , res) {
    try {
        ...
        console.error("Error in getMy friend controller" , error.message);
        res.status(500).json({
            message : "Internal Server error"
        })
    }
...
export async function sendFriendRequest(req ,res){
    try {
        ...
        if(existingRequest){
            return res.status(400).json({
                message: "A friend request already exists bwtween you and this user"
            })
        }
...
export async function acceptFriendRequest(req , res){
    try {
        ...
        //$addToSet : adds element to array only if they do not exists
        ...
        console.log("Error in acceptFriendRequst controller" , error.message)
        res.status(500).json({
            message: "Internal server error"
        })
    }
